...
for idx, filename in enumerate(rs_files):
    ...
    canvas = np.zeros((h + title_height, w, 3), dtype=np.uint8)  # 黑色背景（默认 0）
    canvas[title_height:, :] = concat_img  # 图像贴到底部

    # 写标题文字（居中位置）
    cv2.putText(canvas, "Rolling Shutter", (30, 28), font, font_scale, color_white, thickness, cv2.LINE_AA)
    cv2.putText(canvas, "Corrected", (w // 2 + 30, 28), font, font_scale, color_white, thickness, cv2.LINE_AA)
