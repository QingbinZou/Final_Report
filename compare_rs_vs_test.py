...
# ---------- 模型加载 ----------
model = build_resunet(input_shape=(384, 512, 3))
...
for idx, filename in enumerate(rs_files):
    ...
    canvas = np.zeros((h + title_height, w, 3), dtype=np.uint8)  # 黑色背景
    canvas[title_height:, :] = concat_img  # 图像贴到底部

    # 写标题文字
    cv2.putText(canvas, "Rolling Shutter", (60, 28), font, font_scale, color_white, thickness, cv2.LINE_AA)
    cv2.putText(canvas, "Corrected", (w // 2 + 60, 28), font, font_scale, color_white, thickness, cv2.LINE_AA)
